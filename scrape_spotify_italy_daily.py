...
def getCharts(country_id, charts_date, date_type, random_id):
    ...
    while True:
        try:
            for driver in DriversPool:
                if driver.is_available() and not driver.has_response():
                    driver.get_charts_spotify(country_id, charts_date, date_type)
                    break
            else:
                time.sleep(1)
                print(f'[{random_id}] Waiting for a driver to be available...')
...
def writeCharts(country_id, charts_date, date_type):
    ...
    output_path = f'{output_dir}/output_{category}_{country_id}_{date_type}_{chart_date}.csv'
    ...
    writer.writerow(["position", "title", "artists", "plays", "isrc"])
    output_file.flush()
    ...
    for row in rows:
        ...
        writer.writerow([row['position'], row['title'], ", ".join(row['artists']), row['plays'], row['isrc']])
        output_file.flush()
        print(row)


def scrape_spotify(start_date, end_date):
    global DriversPool

    now = datetime.datetime.now()
    date_string = now.strftime("%Y-%m-%d %H-%M-%S")

    print("======== Starting the App: Spotify ==========")
    ...
    if (run_mode == "date-range"):
        ...
        writeCharts(country, None, dateType)
